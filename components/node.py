...
from components.utils import extract_json_from_text, STATUS_PENDING, STATUS_RUNNING, STATUS_COMPLETED, STATUS_FAILED, STATUS_OVERRIDDEN
...
class Node:
    def __init__(self, parent_id: Optional[str] = None, task_description: Optional[str] = None, depth: int = 0):
        ...
        self.error_message = ""
        self.task_description = task_description  # Store task directly as property too
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "task": self.task_description or self.retrieve_from_memory("task") or "",
    ...
    def build_prompt(self) -> str:
        ...
        task_description = self.task_description or self.retrieve_from_memory("task")
        if task_description:
            prompt.append(f"Task: {task_description}")

        # Add constraints
        if hasattr(st.session_state, 'attention_mechanism') and self.node_id in st.session_state.attention_mechanism.constraints:
